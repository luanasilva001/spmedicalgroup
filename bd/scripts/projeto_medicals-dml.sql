-- Define o banco que será utilizado
use projeto_medicals;
go

-- Insere os tipos de usuários
insert into tipoUsuario (tituloTipoUsuario)
values					('Administrador')
					   ,('Médico')
					   ,('Paciente');
go

-- Insere os usuários
insert into Usuario (idTipoUsuario, email, senha)
values				 
					 (1, 'adm@adm.com', 'adm123')
					,(2, 'ricardo.lemos@spmedicalgroup.com.br', 'ricardo123')
					,(2, 'roberto.possarle@spmedicalgroup.com.br', 'possarle456')
					,(2, 'helena.souza@spmedicalgroup.com.br', 'helena789')
					,(3, 'ligia@gmail.com', 'ligia123')
					,(3, 'alexandre@gmail.com', 'alexandre456')
					,(3, 'fernando@gmail.com', 'fernando789')
					,(3, 'henrique@gmail.com', 'henrique987')
					,(3, 'joao@gmail.com', 'joao654')
					,(3, 'bruno@gmail.com', 'bruno123')
					,(3, 'mariana@outlook.com', 'mariana987')
					
go

-- Insere as especialidades da clinica
insert into especialidade (nomeEspecialidade)
values					  ('Acupuntura')
						 ,('Anestesiologia')
						 ,('Angiologia')
						 ,('Cardiologia')
						 ,('Cirurgia Cardiovascular')
						 ,('Cirurgia de Mão')
						 ,('Cirurgia do Aparelho Digestivo')
						 ,('Cirurgia Geral')
						 ,('Cirurgia Pediátrica')
						 ,('Cirurgia Plástica')
						 ,('Cirurgia Torácica')
						 ,('Cirurgia Vascular')
						 ,('Dermatologia')
						 ,('Radioterapia')
						 ,('Urologia')
						 ,('Pediatria')
						 ,('Psiquiatria');
go

-- Insere a cliníca
insert into clinica (CNPJ, horarioAbertura, horarioFechamento, endereco, nomeFantasia, razaoSocial)
values				('86400902000130', '07:30' , '18:00','Av. Barão Limeira, 532', 'Clinica Possarle', 'SP Médical Group');
go

-- Insere todos os médicos que trabalham na clinica
insert into medico (idUsuario, idEspecialidade, idClinica, nomeMedico,CRM)
values			   (2, 2, 1,'Ricardo Lemos','54356-SP')
				  ,(3, 17, 1, 'Roberto Possarle' ,'53852-SP')
				  ,(4, 16, 1, 'Helena Strada' ,'65463-SP');
go

-- Insere todos os pacientes
insert into paciente (idUsuario, dataNascimento, nomePaciente, RG, CPF, telefone, endereco)
values				 (5, '10/03/1983', 'Ligia','435225435', '94839859000', '1134567654', 'Rua Estado de Israel 240, São Paulo, Estado de São Paulo, 04022-000')
					,(6, '07/03/2001', 'Alexandre','326543457', '73556944057', '11987656543', 'Av. Paulista, 1578 - Bela Vista, São Paulo - SP, 01310-200')
					,(7, '10/10/1978', 'Fernando','546365253', '16839338002', '11972084453', 'Av. Ibirapuera - Indianópolis, 2927,  São Paulo - SP, 04029-200')
					,(8, '13/10/1985', 'Henrique','543663625', '14332654765', '1134566543', 'R. Vitória, 120 - Vila Sao Jorge, Barueri - SP, 06402-030')
					,(9, '27/08/1975', 'João','325444441', '91305348010', '1176566377', 'R. Ver. Geraldo de Camargo, 66 - Santa Luzia, Ribeirão Pires - SP, 09405-380')
					,(10, '21/03/1972', 'Bruno','545662667', '79799299004', '11954368769', 'Alameda dos Arapanés, 945 - Indianópolis, São Paulo - SP, 04524-001')
					,(11, '05/03/2018','Mariana','545662668', '13771913039', null, 'R Sao Antonio, 232 - Vila Universal, Barueri - SP, 06407-140');
go

-- Insere a situacao da consulta do paciente
insert into situacao (situacao)
values				 ('Realizada')
					,('Cancelada')
					,('Agendada');
go

-- Insere a consulta do paciente
insert into consulta (idMedico, idPaciente, idSituacao, dataConsulta, descricao)
values				 (3, 7, 1, '14/04/2021', 'Criança com catarro na garganta')
					,(2, 2, 2, '28/03/2021', 'Paciente com falta de confiança em si mesmo')
					,(2, 3, 1, '29/03/2021', 'Paciente com depressão severa')
					,(2, 2, 1, '08/04/2021', 'Paciente com boderline')
					,(1, 4, 2, '21/05/2021', 'Paciente verificando se tem alergia a anestesia utilizada na cirurgia')
					,(3, 7, 3, '30/03/2021', 'Criança com dor de bronquilote')
					,(1, 4, 3, '06/04/2021', 'Paciente com parestesia')
					,(1, 4, 2, '09/10/2002', '');
go

-- Insere a consulta do paciente sem a descrição para testar o default
insert into  consulta (idMedico, idPaciente, idSituacao, dataConsulta)
values (3, 2, 1, '15/08/2019');
go

-- Troca a data de nascimento de um determinado paciente
update paciente
set dataNascimento = '26/10/2002'
where idPaciente = 2;